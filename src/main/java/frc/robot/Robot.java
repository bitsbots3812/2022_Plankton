...
public class Robot extends TimedRobot {
  ...
  */
  ...
  ColorSensorInterface colorSensor = new ColorSensorInterface(new ColorSensorV3(I2C.Port.kOnboard), 100);
...
*/
  ...
  public void robotInit() {
    ...
    leds.setAllSolid( 0,1,0, false);

    fireControl.setShooterTargetRPM(2700);
  }
  ...
  public void autonomousPeriodic() {
    if (limelight.distToTarget(currentContstants) < 3 && !autonomousTimer.hasElapsed(5)) {
...
*/
    ...
    if (currentManipulatorControlsMap.getButtonPressed(currentManipulatorControlsMap.PIXY_TRACK_BUTTON) && pixy.steeringSuggestion(currentState.selectedColor) != 0) {
    ...
    if (currentManipulatorControlsMap.getButtonReleased(currentManipulatorControlsMap.PIXY_TRACK_BUTTON) || 
    ...
    switch (currentState.currentLockMode) {
      case UNLOCKED: {
        ...
        if (currentState.isLoaded){
          if (colorSensor.getColor() == BallColor.RED_BALL) {
            leds.setAllSolid(127, 0, 0, true);
          }
          else {
            leds.setAllSolid(0, 0, 127, true);
          }
        } 
        else {
          leds.setAllSolid( 127,15,0, true);
        }
      ...
      case PIXY: {
        ...
        if (currentState.selectedColor == PixyInterface.BallColor.RED_BALL) {
          leds.setAllSolid(127, 0, 0, false);
        }
        else {
          leds.setAllSolid(0, 0, 127, false);
        }
        break;
      }
      case LIMELIGHT: {
        ...
        if (limelight.distToTarget(currentContstants) <= 2.6) {
          leds.setAllSolid(0, 127, 0, false);
        }
        else {
          leds.setAllSolid(127, 127, 127, false);
        }
        break;
      }
    ...
    //SmartDashboard.putNumber("NAV-X Yaw", ahrsRotation);
